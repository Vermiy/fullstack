...
export default function SignUp() {
  ...
  const { mutate, isPending, isError, error } = useMutation({
    mutationFn: (data: { name: string; email: string; password: string }) => SignUpService(data),
  ...
  return (
    <section className="mx-auto flex min-h-[70vh] w-full max-w-md items-center px-6 py-12">
      <div className="w-full rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-7 shadow-[0_14px_30px_rgba(117,74,28,0.1)]">
        ...
        <p className="mt-1 text-sm text-[#6e5438]">Create your warm reading space.</p>
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          <div>
            <label className="block text-sm font-medium text-[#5d462f]">Name</label>
            <input
              type="text"
              value={name}
              onChange={(e) => setName(e.target.value)}
              disabled={isPending}
              className="mt-1 w-full rounded-lg border border-[var(--border)] bg-white px-4 py-2.5 text-[var(--foreground)] transition outline-none focus:border-[var(--accent)] focus:ring-2 focus:ring-[#e6d0af] disabled:opacity-50"
          ...
          <div>
            <label className="block text-sm font-medium text-[#5d462f]">Confirm Password</label>
            <input
              type="password"
